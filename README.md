...
1. Open your Terminal and navidate to a folder `Blockchain-Tools`. Switch from base to ethereum environment (`conda activate ethereum`). After, you will generate two new nodes with new account addresses that will serve as our pre-approved sealer addresses.
    ...
    * Create accounts for two nodes for the network with a separate `datadir` for each using `geth`: (mine are node 10 & 11, you can name yours 1 & 2)

        * ./geth --datadir node10 account new (it will give you 'Public address of the key': 0x8781463c74dDe09aFb77F4E89e4516e1B2C41eF9 and 'Path of the secret key file'. Save both in a notebook for your reference. You will need a "Public address" later.)
        * ./geth --datadir node11 account new (it will give you 'Public address of the key': 0x3345dAed82bB089218d75839D9d347f91Fb89490 and 'Path of the secret key file'. Save both in a notebook for your reference. You will need a "Public address" later.)


2. Next, generate your genesis block.

    * Run `./puppeth` command in your Terminal, name your network (in my case, I named it "bzeth"), and select the option to configure a new genesis block. 
    ...
    * Add Chain ID. It can be any random number. In my case: 2021.

    * Complete the rest of the prompts, and when you are back at the main menu, choose the "Manage existing genesis" option.

    * Export genesis configurations. This will fail to create two of the files, but you only need `bzeth.json`.

3. With the genesis block creation completed, you will now initialize the nodes with the genesis' json file.

    * Using `geth`, initialize each node with the new `bzeth.json`. (again, mine are 10 & 11, you can name yours 1 & 2)
        * ./geth --datadir node10 init bzeth.json
        * ./geth --datadir node11 init bzeth.json

4. Now the nodes can be used to begin mining blocks.

    * Run the nodes in separate terminal windows with the commands: (again, mine are 10 & 11, you can name yours 1 & 2)
        *  ./geth --datadir node10 --unlock "8781463c74dDe09aFb77F4E89e4516e1B2C41eF9" --mine --rpc --allow-insecure-unlock --syncmode full
        *  ./geth --datadir node11 --unlock "3345dAed82bB089218d75839D9d347f91Fb89490" --mine --port 30306 --bootnodes "enode://ee8edadef661a035cc922f22de0c37c2db314c26963a1d6e1aa6c6a010823b0647988716543bf73550076964729330686a5a5290bab069439a29897396fea2f2@127.0.0.1:30303" --ipcdisable --allow-insecure-unlock --syncmode full
...
6. With both nodes up and running, the blockchain can be added to MyCrypto for testing.
    ...
    * Make sure that you scroll down to choose `Custom` in the "Network" column to reveal more options like `Chain ID`. In my case, my chain id was 2021:
...
7. After connecting to the custom network in MyCrypto, it can be tested by sending money between accounts.
    ...
    * In the `To Address` box, type the account address from Node11, then fill in an arbitrary amount of ETH:
